...
def render_config_with_comments(config: dict) -> str:
    ...
    lines = [
        ...
        "# interface: SocketCAN 接口名，如 can0；socketcan 时必填",
        "# selector: dameow 设备选择器，可填设备索引/SN/别名；仅 dameow 生效",
        "# tx_id: 发送到下位机的 CAN ID，建议使用十六进制字符串",
        "# bus_mode: can 或 canfd；当前 33 字节 payload 想保持不变时，应使用 canfd",
        "# bitrate/data_bitrate: socketcan 时需先在系统中把接口按相同参数拉起；dameow 时会传给 drv_ws",
        "# channel/device_type: dameow 通道与设备类型，device_type 默认 0=DEV_USB2CANFD，仅 dameow 生效",
        "# bridge_library: drv_ws bridge 动态库路径，仅 dameow 生效；留空则自动搜索 drv_ws_bridge/build/libdrv_ws_bridge.so",
        "# bitrate_switch: CAN FD 是否启用 BRS",
        "can:",
        f"  enabled: {str(bool(can.get('enabled', False))).lower()}",
        f"  driver: \"{can.get('driver', 'socketcan')}\"",
